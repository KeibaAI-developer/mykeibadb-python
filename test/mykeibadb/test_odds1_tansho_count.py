"""odds1_tanshoテーブルのデータ件数検証テスト.

年間登録出走頭数（出走取消・発走除外含む）と、mykeibadbのodds1_tanshoテーブルの
...
def _get_odds1_tansho_count(conn: psycopg2.extensions.connection, year: str) -> int:
    """指定年の中央競馬の延べ出走頭数をodds1_tanshoテーブルから取得."""
    query = """
...
def test_odds1_tansho_count_matches_jra_official(
    db_connection: psycopg2.extensions.connection, year: str, expected_count: int
) -> None:
    """odds1_tanshoの出走頭数がJRA公式発表と一致することを確認."""
    actual_count = _get_odds1_tansho_count(db_connection, year)
    assert (
        actual_count == expected_count
    ), f"{year}年: odds1_tanshoの出走頭数が{actual_count}件（期待値: {expected_count}件）"
