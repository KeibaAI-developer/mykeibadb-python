...
## 1. `mykeibadb/analytics/_models.py` (L88-L94)

**投稿者**: Copilot | **投稿日時**: 2026-06-04T09:12:47Z
...
AttrSource.from_dict が入力 dict の値をそのまま代入しているため、top_n/kyori が str や None のまま入り得て、型注釈（int / int | None）と実体が不整合になります。後続ロジックで数値演算・比較を行う前提なら、ここで int への変換（および None の扱い）を確定させてください。

---

## 2. `mykeibadb/analytics/_models.py` (L121-L127)

**投稿者**: Copilot | **投稿日時**: 2026-06-04T09:12:48Z
...
EntryAttrDef.from_dict の rows パースが、想定外の型（例: 長さ2以外の list / dict など）でもそのまま RowsDef に入れてしまい、後続で CASE 式生成等をする際に扱えない値が混入し得ます。RowsDef の仕様（(min,max)・単一 int/str）に沿わない値はここで ValueError にして早期に検出した方が安全です。

---

## 3. `mykeibadb/analytics/_cte_helpers.py` (L18-L21)
...
week_in_course の説明が「1始まり」になっていますが、テストでは week_in_course=0 を許容しており（準正常系）、実装的にも 0 指定は単にヒットなしになる挙動です。利用者が混乱しないよう、この引数の許容範囲（0を許すのか／1以上必須なのか）を docstring 上で実装・テストと一致させてください。

---

## 4. `doc/DEV_HISTORY/SPEC.md` (L195-L200)
...
実装要件で参照しているヘルパー名が `_build_course_week_cte` になっていますが、このPRで追加されている関数名は `build_course_week_cte` です。後続PRで仕様書どおりに実装すると名前の不一致で混乱するので、仕様書側を実装に合わせて修正してください。

---

## 5. `doc/DEV_HISTORY/SPEC.md` (L323-L328)
...
SPEC.md の CTE ヘルパー仕様の `build_course_week_cte` シグネチャ／戻り値が、実装（keibajo + cte_params を受け取り、(cte_sql, join_sql) を返す）と一致していません。後続PRの実装指針として使うドキュメントなので、このセクションは実装とテストに合わせて更新してください。
