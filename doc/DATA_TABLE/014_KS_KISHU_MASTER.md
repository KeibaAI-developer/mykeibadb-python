...
| RECORD_SHUBETSU_ID |   | レコード種別ID | 2.0 | KS をセットレコードフォーマットを特定する |
...
| MASSHO_KUBUN |   | 騎手抹消区分 | 1.0 | 0:現役 1:抹消 |
| MENKYO_KOFU_NENGAPPI |   | 騎手免許交付年月日 | 8.0 | 年4桁(西暦)＋月日各2桁 yyyymmdd 形式 |
| MENKYO_MASSHO_NENGAPPI |   | 騎手免許抹消年月日 | 8.0 | 年4桁(西暦)＋月日各2桁 yyyymmdd 形式 |
| SEINENGAPPI |   | 生年月日 | 8.0 | 年4桁(西暦)＋月日各2桁 yyyymmdd 形式 |
| KISHUMEI |   | 騎手名 | 34.0 | 全角17文字　姓＋全角空白1文字＋名　外国人の場合は連続17文字 |
|   |   | 予備 | 34.0 |   |
| KISHUMEI_HANKAKU_KANA |   | 騎手名半角ｶﾅ | 30.0 | 半角30文字　姓15文字＋名15文字　外国人の場合は連続30文字 |
| KISHUMEI_RYAKUSHO |   | 騎手名略称 | 8.0 | 全角4文字 |
| KISHUMEI_ENG |   | 騎手名欧字 | 80.0 | 半角80文字　姓＋半角空白1文字＋名　フルネームで記載 |
| SEIBETSU_KUBUN |   | 性別区分 | 1.0 | 1:男性　2:女性 |
| KIJO_SHIKAKU_CODE |   | 騎乗資格コード | 1.0 | <コード表 2302.騎乗資格コード>参照 |
| KISHU_MINARAI_CODE |   | 騎手見習コード | 1.0 | <コード表 2303.騎手見習コード>参照 |
| TOZAI_SHOZOKU_CODE |   | 騎手東西所属コード | 1.0 | <コード表 2301.東西所属コード>参照 |
| SHOTAI_CHIIKIMEI |   | 招待地域名 | 20.0 | 全角10文字 |
| SHOZOKU_CHOKYOSHI_CODE |   | 所属調教師コード | 5.0 | 騎手の所属厩舎の調教師コード、フリー騎手の場合はALL0を設定 |
| SHOZOKU_CHOKYOSHIMEI_RYAKUSHO |   | 所属調教師名略称 | 8.0 | 全角4文字 |
|   |   | <初騎乗情報> | 67.0 | 平地初騎乗・障害初騎乗の順に設定 |
| HATSUKIJO1_RACE_CODE |   | 年月日場回日R | 16.0 | レース詳細のキー情報 |
| HATSUKIJO1_SHUSSO_TOSU |   | 出走頭数 | 2.0 | 登録頭数から出走取消と競走除外･発走除外を除いた頭数 |
| HATSUKIJO1_KETTO_TOROKU_BANGO |   | 血統登録番号 | 10.0 | 生年(西暦)4桁＋品種1桁<コード表2201.品種コード>参照＋数字5桁 |
| HATSUKIJO1_BAMEI |   | 馬名 | 36.0 | 全角18文字 |
| HATSUKIJO1_KAKUTEI_CHAKUJUN |   | 確定着順 | 2.0 |   |
| HATSUKIJO1_IJOKUBUN_CODE |   | 異常区分コード | 1.0 | <コード表 2101.異常区分コード>参照 |
| HATSUKIJO2_RACE_CODE |   | 年月日場回日R | 16.0 | レース詳細のキー情報 |
| HATSUKIJO2_SHUSSO_TOSU |   | 出走頭数 | 2.0 | 登録頭数から出走取消と競走除外･発走除外を除いた頭数 |
| HATSUKIJO2_KETTO_TOROKU_BANGO |   | 血統登録番号 | 10.0 | 生年(西暦)4桁＋品種1桁<コード表2201.品種コード>参照＋数字5桁 |
| HATSUKIJO2_BAMEI |   | 馬名 | 36.0 | 全角18文字 |
| HATSUKIJO2_KAKUTEI_CHAKUJUN |   | 確定着順 | 2.0 |   |
| HATSUKIJO2_IJOKUBUN_CODE |   | 異常区分コード | 1.0 | <コード表 2101.異常区分コード>参照 |
|   |   |   |   |   |
|   |   | <初勝利情報> | 64.0 | 平地初騎乗・障害初騎乗の順に設定 |
| HATSUSHORI1_RACE_CODE |   | 年月日場回日R | 16.0 | レース詳細のキー情報 |
| HATSUSHORI1_SHUSSO_TOSU |   | 出走頭数 | 2.0 | 登録頭数から出走取消と競走除外･発走除外を除いた頭数 |
| HATSUSHORI1_KETTO_TOROKU_BANGO |   | 血統登録番号 | 10.0 | 生年(西暦)4桁＋品種1桁<コード表2201.品種コード>参照＋数字5桁 |
| HATSUSHORI1_BAMEI |   | 馬名 | 36.0 | 全角18文字 |
| HATSUSHORI2_RACE_CODE |   | 年月日場回日R | 16.0 | レース詳細のキー情報 |
| HATSUSHORI2_SHUSSO_TOSU |   | 出走頭数 | 2.0 | 登録頭数から出走取消と競走除外･発走除外を除いた頭数 |
| HATSUSHORI2_KETTO_TOROKU_BANGO |   | 血統登録番号 | 10.0 | 生年(西暦)4桁＋品種1桁<コード表2201.品種コード>参照＋数字5桁 |
