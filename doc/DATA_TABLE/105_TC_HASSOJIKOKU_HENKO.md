...
| RECORD_SHUBETSU_ID |   | レコード種別ID | 2.0 | TC をセットレコードフォーマットを特定する | TC |
| DATA_KUBUN |   | データ区分 | 1.0 | 1:初期値 | 1 |
| DATA_SAKUSEI_NENGAPPI |   | データ作成年月日 | 8.0 | 西暦4桁＋月日各2桁 yyyymmdd 形式 | 20260201 |
| RACE_CODE | ◯ | レースコード | 16.0 | 開催年+月日+競馬場コード+回次+日次+レース番号 | 2026020105010210 |
...
| RACE_BANGO |   | レース番号 | 2.0 | 該当レース番号 | 10 |
| HAPPYO_TSUIKIHI_JIFUN |   | 発表月日時分 | 8.0 | 月日時分各2桁 | 02010820 |
|   |   | <変更後情報> |   |   |  |
| HENKOGO_HASSO_JIKOKU |   | 変更後 発走時刻 | 4.0 | 時分各2桁 hhmm形式 | 1501 |
|   |   | <変更前情報> |   |   |  |
| HENKOMAE_HASSO_JIKOKU |   | 変更前 発走時刻 | 4.0 | 時分各2桁 hhmm形式 | 1500 |

## 追加カラム一覧

`SokuhoGetter.get_hassojikoku_henko()` メソッドではデフォルトで`convert_codes=True`が指定されており、以下のカラムが追加されます。
