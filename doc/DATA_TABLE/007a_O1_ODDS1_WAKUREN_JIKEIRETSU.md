# ODDS1_WAKUREN_JIKEIRETSU

- レコード名: オッズ1(単複枠) 時系列
- サブレコード: オッズ1(単複枠) 枠連

## カラム一覧

mykeibadb公式ドキュメントに則りカラム名を大文字で記載していますが、データフレームとして取得した際はカラム名は小文字になります。

| 名前 | キー | 項目名 | バイト | 説明 | 例 |
| --- | --- | --- | --- | --- | --- |
| INSERT_TIMESTAMP |   | テーブル作成時間 | 19.0 |   | 2026-01-22 14:05:50 |
| UPDATE_TIMESTAMP |   | テーブル更新時間 | 19.0 | (現在未使用) | 0000-00-00 00:00:00 |
| RECORD_SHUBETSU_ID |   | レコード種別ID | 2.0 | O1(オー･イチ) をセットレコードフォーマットを特定する | O1 |
| DATA_KUBUN |   | データ区分 | 1.0 | 1:中間　2:前日売最終　3:最終　4:確定　5:確定(月曜) | 1 |
|   |   |   |   | 9:レース中止　0:該当レコード削除(提供ミスなどの理由による) |  |
| DATA_SAKUSEI_NENGAPPI |   | データ作成年月日 | 8.0 | 西暦4桁＋月日各2桁 yyyymmdd 形式 | 20251227 |
| RACE_CODE | ○ | レースコード | 16.0 | 開催年+月日+競馬場コード+回次+日次+レース番号 | 2025122806050811 |
| KAISAI_NEN |   | 開催年 | 4.0 | 該当レース施行年 西暦4桁 yyyy形式 | 2025 |
| KAISAI_GAPPI |   | 開催月日 | 4.0 | 該当レース施行月日 各2桁 mmdd形式 | 1228 |
| KEIBAJO_CODE |   | 競馬場コード | 2.0 | 該当レース施行競馬場 <コード表 2001.競馬場コード>参照 | 06 |
| KAISAI_KAIJI |   | 開催回[第N回] | 2.0 | 該当レース施行回 その競馬場でその年の何回目の開催かを示す | 05 |
| KAISAI_NICHIJI |   | 開催日目[N日目] | 2.0 | 該当レース施行日目 そのレース施行回で何日目の開催かを示す | 08 |
| RACE_BANGO |   | レース番号 | 2.0 | 該当レース番号 | 11 |
| HAPPYO_TSUKIHI_JIFUN | ○ | 発表月日時分 | 8.0 |   | 12270654 |
|   |   | <枠連オッズ> | 9.0 | 組番昇順　1-1～8-8 |  |
| KUMIBAN |  ○ | 組番 | 2.0 | 該当枠番 | 78 |
| ODDS |   | オッズ | 5.0 | 9999.9倍で設定　ただし2004年8月13日以前は999.9倍が設定できる最高値とする | 01632 |
|   |   |   |   | “09999”:999.9倍以上　”00000″:無投票　”—–“:発売前取消 |  |
| NINKI |   | 人気順 | 2.0 | “\*\*\*\*\*”:発売後取消　” “:登録なし(sp) | 26 |
|   |   |   |   | スペース:登録なし ‘–‘:発売前取消 ‘\*\*’:発売後取消 無投票の時は発売されている組合せの最大値を設定 |  |

## 追加カラム一覧

`OddsGetter.get_odds1_wakuren_jikeiretsu()` メソッドではデフォルトで`convert_codes=True`が指定されており、以下のカラムが追加されます。

|名前|項目名|説明|例|
|:----|:----|:----|----|
|keibajo|競馬場名|KEIBAJO_CODEを場略名(3文字)に変換 <コード表 2001.競馬場コード>参照| 中山 |
