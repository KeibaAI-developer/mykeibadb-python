...
|   |   | <単勝票数> | 15.0 | 28頭立てまで考慮し繰返し28回　馬番昇順01～28 |  |
| UMABAN | ○ | 馬番 | 2.0 | 該当馬番 | 01 |
| HYOSU |   | 票数 | 11.0 | 単位百円 ALL0:発売前取消し、発売票数なし　スペース:登録なし | 00001766563 |
| NINKI |   | 人気順 | 2.0 | スペース:登録なし ‘–‘:発売前取消 ‘\*\*’:発売後取消 | 07 |

## 追加カラム一覧

`HyosuGetter.get_hyosu1_tansho()` メソッドではデフォルトで`convert_codes=True`が指定されており、以下のカラムが追加されます。
