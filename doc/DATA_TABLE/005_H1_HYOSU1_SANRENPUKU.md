...
| HYOSU |   | 票数 | 11.0 | 単位百円 ALL0:発売前取消し、発売票数なし　スペース:登録なし | 00000380804 |
| NINKI |   | 人気順 | 3.0 | スペース:登録なし ‘—‘:発売前取消 ‘\*\*\*’:発売後取消 | 079 |

## 追加カラム一覧

`HyosuGetter.get_hyosu1_sanrenpuku()` メソッドではデフォルトで`convert_codes=True`が指定されており、以下のカラムが追加されます。
