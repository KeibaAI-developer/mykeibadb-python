...
| HAPPYO_TSUKIHI_JIFUN | ○ | 発表月日時分 | 8.0 | 発表月日時分 | 12261829 |
|   |   | <単勝オッズ> | 8.0 | 28頭立てまで考慮し繰返し28回　馬番昇順01～28 |  |
| UMABAN |  ○ | 馬番 | 2.0 | 該当馬番 | 01 |
| ODDS |   | オッズ | 4.0 | 999.9倍で設定 | 0000 |
|   |   |   |   | “9999”:999.9倍以上　”0000″:無投票　”—-“:発売前取消　”\*\*\*\*”:発売後取消　” “:登録なし(sp) |  |
| NINKI |   | 人気順 | 2.0 | スペース:登録なし ‘–‘:発売前取消 ‘\*\*’:発売後取消 無投票の時は発売されている組合せの最大値を設定 | 16 |

## 追加カラム一覧

`OddsGetter.get_odds1_tansho_jikeiretsu()` メソッドではデフォルトで`convert_codes=True`が指定されており、以下のカラムが追加されます。
