...
| RECORD_SHUBETSU_ID |   | レコード種別ID | 2.0 | JGをセットレコードフォーマットを特定する |
...
| BAMEI |   | 馬名 | 36.0 | 全角18字 |
| SHUTSUBA_TOHYO_UKETSUKE | ○ | 出馬投票受付順番 | 3.0 | 同一業務日付で、同一馬を受付（出馬投票の追加、再投票）した順番 |
| SHUSSO_KUBUN |   | 出走区分 | 1.0 | 1:投票馬 2:締切での除外馬 4:再投票馬 5:再投票除外馬 |
|   |   |   |   | 6:馬番を付さない出走取消馬 9:取消馬 |
| JOGAI_JOTAI_KUBUN |   | 除外状態区分 | 1.0 | 1:非抽選馬 |
|   |   |   |   | 2:非当選馬 |